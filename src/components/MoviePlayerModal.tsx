...
export const MoviePlayerModal: React.FC<MoviePlayerModalProps> = ({
  ...
  const playerUrl = useMemo(() => {
    if (poster.mediaType === 'anime') {
      // Anime player: https://cinemaos.live/animeplayer/watch/{id}/{e}
      const animeId = poster.anilistId || poster.tmdbId || 42013;
      return `https://cinemaos.live/animeplayer/watch/${animeId}/${episode}`;
    }

    if (poster.mediaType === 'tv') {
      // TV Show player: https://cinemaos.in/tv/watch/{id}?s={s}&e={e}
  ...
  // Handle Play Button click from input boxes
  const handleApplySeasonEpisode = (targetSeason?: number, targetEpisode?: number) => {
    sound.playOkClick();
    setIsLoading(true);

    const sNum = targetSeason !== undefined ? targetSeason : parseInt(seasonInput, 10) || 1;
    const maxS = Math.max(totalSeasons, 1);
    const validS = Math.max(1, Math.min(sNum, maxS));

    const maxE = poster.mediaType === 'tv' ? (seasonEpisodesMap[validS] || 10) : (poster.totalEpisodes || 24);
    const eNum = targetEpisode !== undefined ? targetEpisode : parseInt(episodeInput, 10) || 1;
    const validE = Math.max(1, Math.min(eNum, maxE));
  ...
  // Up/Down Arrow step handlers for Season
  const handleSeasonIncrement = () => {
    const current = parseInt(seasonInput, 10) || season;
    const maxS = Math.max(totalSeasons, 1);
    if (current < maxS) {
      const next = current + 1;
      setSeasonInput(String(next));
      handleApplySeasonEpisode(next, 1);
    }
  };
  ...
  const handleSeasonDecrement = () => {
    ...
    if (current > 1) {
      ...
      handleApplySeasonEpisode(prev, 1);
    }
  };

  // Up/Down Arrow step handlers for Episode
  const handleEpisodeIncrement = () => {
    const current = parseInt(episodeInput, 10) || episode;
    const maxE = episodesInCurrentSeason;
    if (current < maxE) {
      const next = current + 1;
      setEpisodeInput(String(next));
      handleApplySeasonEpisode(season, next);
    }
  };
  ...
  const handleEpisodeDecrement = () => {
    ...
    if (current > 1) {
      ...
      handleApplySeasonEpisode(season, prev);
    }
  };
              ...
              <div className="relative flex items-center bg-black/70 rounded-lg border border-cyan-500/40 hover:border-cyan-400/80 focus-within:border-cyan-400 transition-colors shadow-inner">
                ...
                <div className="flex flex-col border-l border-white/10 pr-0.5">
                  <button
                    ...
                    disabled={parseInt(episodeInput, 10) >= episodesInCurrentSeason}
                    title="Next Episode"
        ...
        {poster.mediaType === 'anime' && (
          <div className="absolute left-1/2 -translate-x-1/2 pointer-events-auto flex items-center gap-2 sm:gap-3">
            ...
            <button
              ...
              type="button"
              onClick={() => handleApplySeasonEpisode(1)}
